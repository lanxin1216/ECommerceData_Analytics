...
public class OrderSalesAnalysisDaoImpl implements OrderSalesAnalysisDao {
    ...
    public List<Map<String, Object>> queryOrderTrend(String type) {
        ...
        String sql = String.format(
                "SELECT %s AS period, COUNT(*) AS count FROM orders GROUP BY %s",
                groupBy, groupBy);
    ...
    public List<Map<String, Object>> queryHotCategoryRanking() {
        String sql = "SELECT category, SUM(total_amount) AS sales FROM orders GROUP BY category ORDER BY sales DESC LIMIT 10";
        return hiveJdbcTemplate.queryForList(sql);
    }

    @Override
    public List<Map<String, Object>> queryRegionSales() {
        String sql = "SELECT delivery_province AS region, SUM(total_amount) AS sales FROM orders GROUP BY delivery_province";
        return hiveJdbcTemplate.queryForList(sql);
    }
    ...
    public List<Map<String, Object>> queryAgeGroupConsumption() {
        String sql = "SELECT CASE " +
                     ...
                     "ELSE '60+' END AS age_group, SUM(total_amount) AS total FROM orders GROUP BY age_group";
        return hiveJdbcTemplate.queryForList(sql);
    }

    @Override
    public List<Map<String, Object>> queryMemberComparison() {
        String sql = "SELECT is_member, COUNT(*) AS count, SUM(total_amount) AS total FROM orders GROUP BY is_member";
        return hiveJdbcTemplate.queryForList(sql);
    }
}